...
class ParameterUpdater:
    ...
    def get_file_permissions(self, file_path: str) -> Tuple[int, int, int]:
        ...
        try:
            ...
            self.logger.error(
                f"Fehler beim Ermitteln der Berechtigungen für {file_path}: {e}"
            )
            raise

    def restore_file_permissions(
        self, file_path: str, permissions: int, uid: int, gid: int
    ):
        """
        ...
        try:
            ...
            self.logger.info(
                f"Besitzverhältnisse wiederhergestellt: UID={uid}, GID={gid}"
            )
            ...
            self.logger.error(
                f"Fehler beim Wiederherstellen der Berechtigungen für {file_path}: {e}"
            )
            raise
    ...
    def update_parameters(
        self, file_path: str, new_parameters: Dict[str, Any], create_backup: bool = True
    ) -> bool:
        """
...
Beispiele:
  ...
  python parameter_updater.py --generate-config
    ...
    parser.add_argument("--file", "-f", help="Pfad zur parameters.yml Datei")
    parser.add_argument(
        "--param",
        "-p",
        action="append",
    ...
    parser.add_argument(
        "--config", "-c", help="Pfad zur Konfigurationsdatei mit Server-Definitionen"
    )
    parser.add_argument(
        "--generate-config",
        action="store_true",
    ...
    parser.add_argument(
        "--no-backup", action="store_true", help="Kein Backup erstellen"
    )
    parser.add_argument(
        "--validate", "-v", help="Validiert nur die YAML-Syntax einer Datei"
    )
    parser.add_argument(
        "--log-level",
        default="INFO",
    ...
    if args.generate_config:
        ...
        print(
            "Beispiel-Konfiguration wurde in 'server_config_example.yml' gespeichert."
        )
        return
    ...
    if args.file and args.param:
        ...
        success = updater.update_parameters(
            args.file, parameters, create_backup=not args.no_backup
        )
