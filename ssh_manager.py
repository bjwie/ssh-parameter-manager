...
class SSHManager:
    ...
    def get_ssh_connection(self, server_name: str) -> paramiko.SSHClient:
        ...
        try:
            ...
            self.logger.info(f"SSH-Verbindung hergestellt: {server_name} ({server_config['host']})")
    ...
    def execute_remote_command(self, server_name: str, command: str) -> Tuple[str, str, int]:
        ...
        try:
            ...
            self.logger.error(f"Fehler beim Ausführen des Befehls auf {server_name}: {e}")
            raise
    ...
    def download_file(self, server_name: str, remote_path: str, local_path: str) -> bool:
        ...
        try:
            with SCPClient(ssh.get_transport()) as scp:
                ...
                self.logger.info(f"Datei heruntergeladen: {remote_path} -> {local_path}")
                return True
    ...
    def get_file_permissions(self, server_name: str, file_path: str) -> Optional[Dict[str, Any]]:
        ...
        self.logger.error(f"Konnte Berechtigungen nicht ermitteln für {file_path}: {stderr}")
        return None
    
    def restore_file_permissions(self, server_name: str, file_path: str, permissions: Dict[str, Any]) -> bool:
        """Stellt Dateiberechtigungen auf Remote-Server wieder her."""
        try:
            # Besitzer und Gruppe wiederherstellen
            chown_cmd = f"chown {permissions['owner']}:{permissions['group']} '{file_path}'"
            stdout, stderr, exit_code = self.execute_remote_command(server_name, chown_cmd)
            ...
            stdout, stderr, exit_code = self.execute_remote_command(server_name, chmod_cmd)
    ...
    def create_remote_backup(self, server_name: str, file_path: str) -> Optional[str]:
        ...
        backup_dir = self.config.get('ssh_settings', {}).get('backup_dir', '/tmp/symfony_backups')
    ...
    def update_remote_parameters(self, server_name: str, customer_name: str, 
        ...
        try:
            ...
            self.logger.info(f"Aktualisiere {customer_name} auf {server_name}: {remote_path}")
            
            # 1. Aktuelle Berechtigungen speichern
            permissions = self.get_file_permissions(server_name, remote_path)
            if not permissions:
                self.logger.warning("Konnte Berechtigungen nicht ermitteln, fahre trotzdem fort...")
            ...
            # 4. Lokal bearbeiten
            success = self.updater.update_parameters(temp_path, new_parameters, create_backup=False)
            if not success:
                os.unlink(temp_path)
                return False
            ...
            self.logger.info(f"Parameter erfolgreich aktualisiert: {customer_name} auf {server_name}")
            return True
    ...
    def bulk_update_parameter(self, targets: List[str], parameter_name: str, 
                            parameter_value: Any) -> Dict[str, bool]:
    ...
    def apply_bulk_template(self, template_name: str, targets: List[str]) -> Dict[str, bool]:
    ...
    def download_all_configs(self, output_dir: str = 'downloaded_configs') -> Dict[str, bool]:
        ...
        for server_name, server_config in self.config['servers'].items():
            for customer_name, customer_config in server_config['customers'].items():
                ...
                results[target] = self.download_file(server_name, remote_path, local_path)
...
Beispiele:
  ...
  python ssh_manager.py --generate-web-config
    ...
    parser.add_argument('--config', '-c', default='ssh_config.yml',
                       help='SSH-Konfigurationsdatei (default: ssh_config.yml)')
    parser.add_argument('--list', action='store_true',
                       help='Alle verfügbaren Kunden auflisten')
    parser.add_argument('--update', nargs='+',
                       help='Kunden aktualisieren (Format: server:kunde)')
    parser.add_argument('--param', action='append',
                       help='Parameter im Format key=value (kann mehrfach verwendet werden)')
    parser.add_argument('--template',
                       help='Bulk-Template anwenden')
    parser.add_argument('--targets', nargs='+',
                       help='Ziel-Kunden für Template (Format: server:kunde)')
    parser.add_argument('--download-all', action='store_true',
                       help='Alle Parameter-Dateien herunterladen')
    parser.add_argument('--generate-web-config', action='store_true',
    ...
    try:
        ...
        if args.update and args.param:
            ...
            for target in args.update:
                if ':' not in target:
                    print(f"❌ Ungültiges Ziel-Format: {target} (erwartet: server:kunde)")
                    continue
        ...
        if not any([args.list, args.update, args.template, args.download_all, args.generate_web_config]):
            parser.print_help()
